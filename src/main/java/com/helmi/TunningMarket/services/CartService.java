...
import com.helmi.TunningMarket.entities.CheckoutCart;
import org.slf4j.ILoggerFactory;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import com.helmi.TunningMarket.entities.AddtoCart;
import com.helmi.TunningMarket.entities.Produit;
import com.helmi.TunningMarket.repositories.AddToCartRepo;
import com.helmi.TunningMarket.repositories.CheckoutRepo;
...
public class CartServiceImpl implements CartService {
    ...
    @Autowired
    ProduitService proServices;

    private static final Logger logger = LoggerFactory.getLogger(CartServiceImpl.class);

    @Override
    public List<AddtoCart> addCartbyUserIdAndProductId(long produitId, long userId, int qty, double price) throws Exception {
        try {
            if(addCartRepo.getCartByProductIdAnduserId(userId, produitId).isPresent()){
                throw new Exception("Product is already exist.");
            }
            AddtoCart obj = new AddtoCart();
            obj.setQty(qty);
            obj.setUser_id(userId);
           // Produit pro = proServices.getProductsById(productId);
            Produit pro = proServices.getProduitById(produitId);
            obj.setProduit(pro);
