...
public interface CartRepository extends JpaRepository<Cart,Long> {
    @Query("Select cart  FROM Cart cart WHERE cart.user.username=:username and cart.commandes.size=0")
    ...
    @Query("Select cart  FROM Cart cart WHERE cart.produit.user.username=:username and cart.commandes.size <> 0")
