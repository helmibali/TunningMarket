...
public interface AddToCartRepo extends JpaRepository<AddtoCart,Long> {
    @Query("Select sum(addCart.prix) FROM AddtoCart addCart WHERE addCart.user_id=:user_id")
    ...
    @Modifying
    @Transactional
    @Query("update AddtoCart addCart set addCart.qty=:qty,addCart.prix=:price WHERE addCart.id=:cart_id")
